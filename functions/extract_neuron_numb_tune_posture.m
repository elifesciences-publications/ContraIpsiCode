function [ neuron_selected ] = extract_neuron_numb_tune_posture( Data_str, varargin )
...
%         if strcmp(test(1:5),'Neuro')
        if strcmp(test(1:5),'Cell_') || strcmp(test(1:5),'Neuro')
    ...
    for i=1:numb_trials
        ...
        for j=1:length(varargin)
            if TP==varargin{j} && strcmp('TRIAL_G',event_tri_type(1:7))
    ...
    for i=1:numb_files
        ...
        for j=1:length(EVENTS.LABELS)
            ...
            if strcmp('TRIAL_G',event_str(1:7)) 
                pert_time=[pert_time EVENTS.TIMES(j)*1000];
                break
            end
        end
    end
    ...
    neuron_selected=zeros(numb_files,neuron_numb,1001);
    for i=1:numb_files
        start_time=int32(pert_time(i)-1000);
        end_time=int32(pert_time(i)+00);
