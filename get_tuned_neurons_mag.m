...
cd('functions')
[ind_sign ] = anova_each_cell( convolved_H );

%get neuron data as an array NxT where N is neuron number and T is number
%of time points
...
if dup_flag==1
    ...
    ind_comb=intersect(ind_comb,good_indices);
...
data=struct;
data.mag_contra=mag_contra;
data.mag_ipsi=mag_ipsi;
...
% errorbar(mean(mag_contra(ind_resp)),mean(mag_ipsi(ind_resp)),std(mag_contra(ind_resp)),std(mag_contra(ind_resp)),std(mag_ipsi(ind_resp)),std(mag_ipsi(ind_resp)))
...
assert(false)
cd('../')
direct=strcat('figures/',data_type,'/single_cell/tuning');
if dup_flag==1
